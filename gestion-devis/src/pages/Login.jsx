...
export default function Login() {
  ...
  const { login, register, startGuest, user } = useAuth();
  ...
  const [demoExpired] = useState(() => {
    try {
      if (sessionStorage.getItem('gdevis-demo-expired')) {
        sessionStorage.removeItem('gdevis-demo-expired');
        return true;
      }
    } catch {
      /* ignore */
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={submit} className="space-y-4 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
          ...
          <Field label={t('auth.email')}>
            <input
              type="email" className={inputClass} value={email}
              onChange={(e) => setEmail(e.target.value)} placeholder="admin@fish-afric.com" required
            />
          </Field>
          <Field label={t('auth.password')}>
            <input
              type="password" className={inputClass} value={password}
        ...
        <div className="mt-4 rounded-xl border border-stone-200 bg-white/70 p-4 text-xs text-stone-600">
          ...
          <p><span className="font-medium">{t('auth.adminAccount')}</span> : admin@fish-afric.com / admin123</p>
          <p><span className="font-medium">{t('auth.commercialAccount')}</span> : commercial@fish-afric.com / commercial123</p>
        </div>
      </div>
    </div>
  );

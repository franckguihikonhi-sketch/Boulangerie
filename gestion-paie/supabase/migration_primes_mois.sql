-- ===========================================================================
-- PaieCI — migration additive : ciblage par mois des primes (comme les
-- retenues et les heures supplémentaires).
...
-- inchangé pour les primes déjà saisies : elles continuent de s'appliquer à
-- tous les mois de leur période, exactement comme avant cette migration).
--
-- Pourquoi : une prime laissée sans mois s'applique à CHAQUE bulletin de sa
-- période contractuelle. C'est correct pour une prime récurrente (logement,
-- rendement mensuel), mais faux pour une prime exceptionnelle ponctuelle
-- (13ᵉ mois, performance annuelle) : sans ce champ, elle serait comptée à
-- tort chaque mois ET, pire, 12 fois dans l'assiette de l'indemnité de congé
-- payé (règle du 1/12ᵉ sur les 12 mois de référence). Renseigner le mois
-- limite désormais la prime au bulletin de ce mois précis uniquement.
--
-- Ajoute :
--   - primes.mois (date, optionnelle) : même logique que retenues.mois et
--       heures_supplementaires.mois.
--   - save_employee(jsonb) mis à jour pour lire/écrire ce nouveau champ.
-- ===========================================================================

alter table primes add column if not exists mois date;

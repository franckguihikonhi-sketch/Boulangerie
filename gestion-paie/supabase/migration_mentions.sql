-- ===========================================================================
-- PaieCI — migration additive : mentions légales obligatoires du bulletin.
...
-- n'est perdue (colonnes ajoutées avec valeur par défaut, table nouvelle).
--
-- Ajoute :
--   - settings.rccm, settings.compte_contribuable, settings.activite
--       (identification légale complète de l'employeur)
--   - employees.categorie (classification professionnelle du salarié)
--   - table retenues (avances / prêts / oppositions judiciaires par période,
--       déduites du net à payer, mentionnées à part sur le bulletin)
--   - save_employee(jsonb) mis à jour pour lire/écrire ces nouveaux champs.
-- ===========================================================================

alter table settings add column if not exists rccm text not null default '';
alter table settings add column if not exists compte_contribuable text not null default '';
alter table settings add column if not exists activite text not null default '';

alter table employees add column if not exists categorie text not null default '';
...
create index if not exists retenues_periode_id_idx on retenues (periode_id);

...
// créées) avec la marche à suivre et un accès direct à la démonstration.
export default function DbGate({ children }) {
  const { status, error } = useDbStatus();
  const { t } = useI18n();
  const { startGuest, isGuest } = useAuth();
      ...
      <div className="mx-auto max-w-lg py-16 text-center">
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6">
          ...
          <div className="mt-4 flex justify-center gap-2">
            ...
            {!isGuest && (
              <Button variant="secondary" onClick={() => { startGuest(); navigate('/'); }}>
                {t('login.guest')}
              </Button>
            )}
          </div>

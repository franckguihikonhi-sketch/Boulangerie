...
function RegisterPreview({ rows, ym, t, locale }) {
  const html = impotsDocumentHtml(rows, ym, { t, locale });
  const onLoad = (ev) => {
    try {
...
export default function Impots() {
  ...
  return (
    <div>
      <PageTitle>{t('impots.title')}</PageTitle>
      <p className="mb-4 text-sm text-stone-500">{t('impots.subtitle')}</p>
            ...
            <>
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex flex-wrap gap-4 text-sm">
                  ...
                  <span>{t('impots.totalImpotSalaires')} : <strong className="text-brand-700">{formatFCFA(totaux.totalImpotSalaires, locale)}</strong></span>
                  <span>{t('impots.totalFdfp')} : <strong className="text-brand-700">{formatFCFA(totaux.totalFdfp, locale)}</strong></span>
                  <span>{t('impots.totalGeneral')} : <strong>{formatFCFA(totaux.totalGeneral, locale)}</strong></span>
                </div>
              </div>
              <InfoNote>{t('impots.previewNote')}</InfoNote>
              <div className="mt-3">
